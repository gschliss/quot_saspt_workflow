...
- Background-corrected survival curves (compute_background_corrected_survival,
  plot_survival_background_corrected, plot_survival_kaplan_meier_overlay_corrected)
  -- companions to the plain survival functions above that subtract off an
  implied background population (see settings['survival']['background_condition']
  in core/settings.py), written as separate output files alongside the
  uncorrected plots rather than replacing them.
...
def compute_background_corrected_survival(
    traj_csvs: list[str],
    background_traj_csvs: list[str],
    min_length: int = 2,
    max_frame: int = 30,
) -> pd.DataFrame | None:
    """Background-subtracted companion to :func:`compute_life_table_survival`.

    Treats the observed population in *traj_csvs* as a mixture of true
    signal and background contamination, where the background's absolute
    trajectory count and survival shape are both estimated directly from
    *background_traj_csvs* (e.g. an untagged/non-recruiting control imaged
    the same way): if the background condition produces ``n_bg``
    trajectories on its own, exactly ``n_bg`` background trajectories
    (following the background condition's own survival curve) are assumed
    to also be present within the observed population, and are subtracted
    out. At each time point, the implied number of surviving background
    particles (``n_bg * S_bg(t)``) is subtracted from the observed
    surviving count (``n_obs * S_obs(t)``), and the remainder is
    renormalized by the implied signal count (``n_obs - n_bg``):

        S_signal(t) = (n_obs * S_obs(t) - n_bg * S_bg(t)) / (n_obs - n_bg)

    Both life tables are computed over the same ``0..max_frame`` integer
    grid (see :func:`compute_life_table_survival`), so they align without
    interpolation.

    Returns
    -------
    DataFrame with columns time, survival_pct, n_obs, n_bg, n_signal -- or
    None if either input has no trajectory meeting *min_length*, or if
    ``n_bg >= n_obs`` (the correction is undefined: it would imply zero or
    negative true-signal trajectories).
    """
    obs_table = compute_life_table_survival(traj_csvs, min_length=min_length, max_frame=max_frame)
    bg_table = compute_life_table_survival(background_traj_csvs, min_length=min_length, max_frame=max_frame)
    if obs_table is None or bg_table is None:
        return None

    n_obs = int(obs_table["n_at_risk"].iloc[0])
    n_bg = int(bg_table["n_at_risk"].iloc[0])
    n_signal = n_obs - n_bg
    if n_signal <= 0:
        print(
            f"  [survival corrected] implied background count ({n_bg}) >= observed "
            f"count ({n_obs}); cannot subtract -- skipping."
        )
        return None

    obs_survivors = obs_table["survival_pct"].to_numpy() / 100.0 * n_obs
    bg_survivors = bg_table["survival_pct"].to_numpy() / 100.0 * n_bg
    signal_survivors = obs_survivors - bg_survivors
    corrected_pct = np.clip(signal_survivors / n_signal * 100.0, 0.0, 100.0)

    return pd.DataFrame({
        "time": obs_table["time"],
        "survival_pct": corrected_pct,
        "n_obs": n_obs,
        "n_bg": n_bg,
        "n_signal": n_signal,
    })
...
def plot_survival_background_corrected(
    traj_csvs: list[str],
    background_traj_csvs: list[str],
    settings: dict,
    label: str,
    ...
    """Background-corrected companion to :func:`plot_survival_kaplan_meier`.

    Does not touch or replace that function's own ``<label>_survival.pdf``
    output -- written alongside it as ``<label>_survival_corrected.pdf``, so
    the raw (uncorrected) curve is always still available too. See
    :func:`compute_background_corrected_survival` for the subtraction.

    Saved to
    ``settings['io']['plot_directory']/survival_curves/<label>_survival_corrected.pdf``.
    Skipped entirely if the correction can't be computed (see
    :func:`compute_background_corrected_survival`).
    """
    corrected = compute_background_corrected_survival(
        traj_csvs, background_traj_csvs, min_length=min_length, max_frame=max_frame
    )
    if corrected is None:
        print(f"  [survival corrected] {label}: could not compute background-corrected curve; skipping")
        return

    n_obs = int(corrected["n_obs"].iloc[0])
    n_bg = int(corrected["n_bg"].iloc[0])
    n_signal = int(corrected["n_signal"].iloc[0])
    print(
        f"  [survival corrected] {label}: {n_obs} observed - {n_bg} implied "
        f"background ({background_label}) = {n_signal} signal"
    )

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.plot(corrected["time"], corrected["survival_pct"], color="black", linewidth=1)
    ax.set_xlabel("Frames elapsed since first detection")
    ax.set_ylabel("Survival probability (%)")
    ax.set_title(f"{label} (background-corrected, background={background_label})")
    ax.set_ylim(0, 105)
    ax.set_xlim(0, max_frame)
    for spine in ["top", "right"]:
        ax.spines[spine].set_visible(False)
    ...
    output_dirname = os.path.join(settings["io"]["plot_directory"], "survival_curves")
    os.makedirs(output_dirname, exist_ok=True)
    output_filename = os.path.join(output_dirname, f"{label}_survival_corrected.pdf")
    fig.savefig(output_filename, dpi=300, format="pdf", bbox_inches="tight")
    _write_trajectory_count_table(
        output_filename,
        [
            (f"{label} (observed)", n_obs),
            (f"{background_label} (background)", n_bg),
            (f"{label} (signal, corrected)", n_signal),
        ],
    )
    print(f"  Saved {os.path.basename(output_filename)}")
...
def plot_survival_kaplan_meier_overlay_corrected(
    grouped_traj_csvs: dict[str, list[str]],
    background_traj_csvs: list[str],
    background_label: str,
    settings: dict,
    ...
    """Background-corrected companion to
    :func:`plot_survival_kaplan_meier_overlay`.

    One corrected trace per group in *grouped_traj_csvs*, using
    *background_traj_csvs* as the shared background estimate for all of
    them (see :func:`compute_background_corrected_survival`). The
    background group itself (``group_label == background_label``) is
    skipped -- correcting it against itself is undefined. Does not touch or
    replace the uncorrected overlay -- written alongside it as
    ``<plot_label>_survival_overlay_corrected.pdf``.

    Saved to
    ``settings['io']['plot_directory']/survival_curves/<plot_label>_survival_overlay_corrected.pdf``.
    Skipped entirely if no group yields a valid correction.
    """
    fig, ax = plt.subplots(figsize=(8, 5))
    count_rows: list[tuple[str, int]] = []

    for group_label, traj_csvs in sorted(grouped_traj_csvs.items()):
        if group_label == background_label:
            continue
        corrected = compute_background_corrected_survival(
            traj_csvs, background_traj_csvs, min_length=min_length, max_frame=max_frame
        )
        if corrected is None:
            print(f"  [survival overlay corrected] {group_label}: could not compute background-corrected curve; skipping")
            continue

        n_signal = int(corrected["n_signal"].iloc[0])
        print(f"  [survival overlay corrected] {group_label}: {n_signal} implied signal trajectories")
        ax.plot(corrected["time"], corrected["survival_pct"], linewidth=1, label=group_label)
        count_rows.append((group_label, n_signal))

    if not count_rows:
        plt.close(fig)
        print(f"  [survival overlay corrected] no group had a valid correction for {plot_label}; skipping")
        return

    ax.set_xlabel("Frames elapsed since first detection")
    ax.set_ylabel("Survival probability (%)")
    ax.set_title(f"{plot_label} (background-corrected, background={background_label})")
    ax.set_ylim(0, 105)
    ax.set_xlim(0, max_frame)
    ax.legend(fontsize="small", loc="best")
    for spine in ["top", "right"]:
    ...
    output_dirname = os.path.join(settings["io"]["plot_directory"], "survival_curves")
    os.makedirs(output_dirname, exist_ok=True)
    output_filename = os.path.join(output_dirname, f"{plot_label}_survival_overlay_corrected.pdf")
    fig.savefig(output_filename, dpi=300, format="pdf", bbox_inches="tight")
    plt.close(fig)
    _write_trajectory_count_table(output_filename, count_rows)
    print(f"  Saved {os.path.basename(output_filename)}")
