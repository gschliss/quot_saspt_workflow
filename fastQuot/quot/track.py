...
# Distance between two sets of 2D points (used by diffusion_weight_matrix)
from scipy.spatial import distance_matrix

# KD-tree for fast radius queries
from scipy.spatial import cKDTree

# Hungarian algorithm (faster C implementation replacing munkres)
from scipy.optimize import linear_sum_assignment

# Sparse graph connected-components (replaces the pure-Python BFS in helper.py)
...
# Finite substitute for inf entries when calling linear_sum_assignment
_LARGE_COST = 1e9
...
def _traj_loc_adjacency(trajs, locs, search_radius_pxl):
    """
    Return a boolean adjacency matrix (n_trajs x n_locs) where
    entry [i,j] is True iff traj i's last position is within
    search_radius_pxl of loc j. Uses a KD-tree for efficiency.

    """
    n_trajs = len(trajs)
    n_locs = locs.shape[0]
    if n_trajs == 0 or n_locs == 0:
        return np.zeros((n_trajs, n_locs), dtype=bool)
    traj_pos = np.array([t.last_pos() for t in trajs])
    loc_pos = locs[:, 2:4]
    adj = np.zeros((n_trajs, n_locs), dtype=bool)
    tree = cKDTree(loc_pos)
    for ti, neighbors in enumerate(tree.query_ball_point(traj_pos, search_radius_pxl)):
        for li in neighbors:
            adj[ti, li] = True
    return adj

def _connected_components(trajs, locs, search_radius_pxl):
    """
    Find connected components of the bipartite traj-loc adjacency graph
    using scipy.sparse.csgraph rather than the pure-Python BFS in
    quot.helper.  The KD-tree pairs are reused directly so no dense
    adjacency matrix is ever built.

    Returns the same five-tuple as quot.helper.connected_components:
        subgraphs    -- list of dummy (n_t, n_l) arrays (shape only used)
        Ti           -- list of traj-index lists per subgraph
        Li           -- list of loc-index lists per subgraph
        traj_singlets -- traj indices with no locs in radius
        loc_singlets  -- loc indices with no trajs in radius
    """
    n_trajs = len(trajs)
    n_locs = locs.shape[0]

    if n_trajs == 0:
        return [], [], [], [], list(range(n_locs))
    if n_locs == 0:
        return [], [], [], list(range(n_trajs)), []

    traj_pos = np.array([t.last_pos() for t in trajs])
    loc_pos = locs[:, 2:4]

    tree = cKDTree(loc_pos)
    neighbor_lists = tree.query_ball_point(traj_pos, search_radius_pxl)

    # Build edge arrays for the bipartite graph.
    # Trajs occupy nodes 0..n_trajs-1; locs occupy n_trajs..n_trajs+n_locs-1.
    traj_edges, loc_edges = [], []
    for ti, neighbors in enumerate(neighbor_lists):
        for li in neighbors:
            traj_edges.append(ti)
            loc_edges.append(li + n_trajs)

    if not traj_edges:
        # No connections at all
        return [], [], [], list(range(n_trajs)), list(range(n_locs))

    traj_edges = np.array(traj_edges, dtype=np.int32)
    loc_edges  = np.array(loc_edges,  dtype=np.int32)

    n_total = n_trajs + n_locs
    rows = np.concatenate([traj_edges, loc_edges])
    cols = np.concatenate([loc_edges,  traj_edges])
    data = np.ones(len(rows), dtype=np.int8)

    sparse_adj = csr_matrix((data, (rows, cols)), shape=(n_total, n_total))
    _, labels = _scipy_connected_components(sparse_adj, directed=False)

    traj_labels = labels[:n_trajs]
    loc_labels  = labels[n_trajs:]
    ...
    for comp in range(labels.max() + 1):
        t_in = np.where(traj_labels == comp)[0]
        l_in = np.where(loc_labels  == comp)[0]

        has_t = len(t_in) > 0
        has_l = len(l_in) > 0

        if has_t and has_l:
            # Non-trivial subproblem. Only the shape of the returned array
            # is used by the caller, so we skip filling in the content.
            subgraphs.append(np.empty((len(t_in), len(l_in)), dtype=np.int64))
            Ti.append(t_in.tolist())
            Li.append(l_in.tolist())
        elif has_t:
            traj_singlets.extend(t_in.tolist())
        else:
            loc_singlets.extend(l_in.tolist())

    return subgraphs, Ti, Li, traj_singlets, loc_singlets
...
def euclidean_weight_matrix(trajs, locs, pixel_size_um=0.16,
    ...
    *J* when other reconnections are available is *init_cost*.
    ...
    # Use KD-tree to compute distances only for pairs within search radius,
    # leaving all out-of-radius entries as inf.
    if n_traj > 0 and n_locs > 0:
        traj_pos = np.array([t.last_pos() for t in trajs])
        loc_pos = locs[:, 2:4]
        distances = np.full((n_traj, n_locs), np.inf)
        tree = cKDTree(loc_pos)
        for ti, neighbors in enumerate(tree.query_ball_point(traj_pos, search_radius_pxl)):
            if neighbors:
                nb = np.array(neighbors)
                diffs = traj_pos[ti] - loc_pos[nb]
...
def reconnect_hungarian(trajs, locs, locs_array, max_blinks=0,
    ...
    Before running the full solver, a greedy pre-assignment pass
    resolves all unambiguous 1:1 pairs (traj has exactly one
    candidate loc and that loc has exactly one candidate traj).
    The Hungarian solver then runs only on the remaining problem,
    which is smaller.
    ...
    if n_traj==1 and n_locs==1:
        ...
        # Find traj:loc pairs where traj has exactly 1 candidate loc AND
        # that loc has exactly 1 candidate traj. These are unambiguously
        # matched and can be assigned without running the full solver.
        search_radius_pxl = (kwargs.get('search_radius', 2.5) /
                             kwargs.get('pixel_size_um', 0.16))
        candidate = _traj_loc_adjacency(trajs, locs, search_radius_pxl)
        traj_n_cand = candidate.sum(axis=1)   # (n_traj,)
        loc_n_cand = candidate.sum(axis=0)    # (n_locs,)

        greedy_traj = np.zeros(n_traj, dtype=bool)
        greedy_loc = np.zeros(n_locs, dtype=bool)

        for ti in range(n_traj):
            if traj_n_cand[ti] == 1:
                li = int(np.argmax(candidate[ti]))
                if loc_n_cand[li] == 1:
                    trajs[ti].add_index(locs[li, 0], (n_traj, n_locs))
                    greedy_traj[ti] = True
                    greedy_loc[li] = True

        # Indices of trajs and locs not resolved by the greedy pass
        rem_ti = np.where(~greedy_traj)[0]
        rem_li = np.where(~greedy_loc)[0]

        if len(rem_ti) == 0 and len(rem_li) > 0:
            # All trajs assigned greedily; start new trajectories from
            # remaining locs
            for li in rem_li:
                if locs[li, 4] >= min_I0:
                    out.append(Trajectory(locs[li, 0], locs_array,
        ...
        elif len(rem_ti) > 0 and len(rem_li) == 0:
            # All locs assigned greedily; blink remaining trajs
            for ti in rem_ti:
                trajs[ti].blink()

        elif len(rem_ti) > 0 and len(rem_li) > 0:
            # Solve the residual assignment problem with the Hungarian
            # algorithm via scipy (C implementation, much faster than munkres)
            rem_trajs = [trajs[i] for i in rem_ti]
            rem_locs = locs[rem_li, :]
            n_rem_traj = len(rem_trajs)
            n_rem_locs = rem_locs.shape[0]

            W = WEIGHT_MATRIX_METHODS[weight_method](
                rem_trajs, rem_locs, **kwargs)

            # linear_sum_assignment requires finite values; replace inf with
            # a large cost that will never be preferred over init_cost
            W_finite = np.where(np.isinf(W), _LARGE_COST, W)
            row_ind, col_ind = linear_sum_assignment(W_finite)

            for i, j in zip(row_ind, col_ind):

                # traj:loc — guard against solver choosing an originally-inf
                # entry (shouldn't happen when init_cost << _LARGE_COST)
                if (i < n_rem_traj) and (j < n_rem_locs):
                    if not np.isinf(W[i, j]):
                        rem_trajs[i].add_index(rem_locs[j, 0], (n_traj, n_locs))
                    else:
                        rem_trajs[i].blink()

                # traj:(empty)
                elif (i < n_rem_traj) and (j >= n_rem_locs):
                    rem_trajs[i].blink()

                # (empty):loc
                elif (j < n_rem_locs) and (i >= n_rem_traj):
                    if rem_locs[j, 4] >= min_I0:
                        out.append(Trajectory(rem_locs[j, 0], locs_array,
                            (n_traj, n_locs), max_blinks=max_blinks))
...
def track(locs, method="diffusion", search_radius=2.5,
    ...
    for fi in range(start_frame+1, stop_frame):
        ...
        # and localizations in this frame.
            ...
            # Break the bipartite traj-loc graph into independent subproblems
            # using scipy's C-based connected_components (replaces the
            # pure-Python BFS in quot.helper that was the dominant bottleneck).
            subgraphs, Ti, Li, traj_singlets, loc_singlets = \
                _connected_components(active, frame_locs, search_radius_pxl)
