...
def _solve_hessian(H, g, ridge=0.0001):
    """
    Solve (H - ridge*I) @ step = -g for step, with ridge retry on
    singularity.  More efficient than invert_hessian when only the
    Newton step vector is needed (avoids materialising the full inverse).

    args
    ----
        H       :   2D ndarray, shape (n, n), the Hessian
        g       :   1D ndarray, shape (n,), the gradient
        ridge   :   float, initial regularization term

    returns
    -------
        1D ndarray, shape (n,), the update step
    """
    D = np.diag(np.ones(H.shape[0]) * ridge)
    while True:
        try:
            return np.linalg.solve(H - D, -g)
...
def fit_ls_int_gaussian(img, guess, sigma=1.0, ridge=0.0001, 
    ...
    while iter_idx<max_iter and (np.abs(update[:2])>convergence).any():
        ...
        # Hessian (not yet inverted)
        H = -J.T @ J / sig2

        # Solve for the Newton step directly — avoids materialising H_inv
        # on every iteration.  invert_hessian is called only once at the end.
        update[:] = _solve_hessian(H, grad, ridge=ridge)

        # Apply the update to parameter estimate
        pars = pars + damp * update
        iter_idx += 1

    # Compute H_inv once at the end for error / determinant estimates
